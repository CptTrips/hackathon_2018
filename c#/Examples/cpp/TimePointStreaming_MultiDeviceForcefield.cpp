...
public:
    ...
    void onFrame(const Leap::Controller &controller) override
    {
        ...
        if (hands.isEmpty())
        {
            ...
            local_hand_data.palm_normal = Ultrahaptics::Vector3();
        ...
        else
        {
            ...
            if (device_palm_position.x >= -28.0f * Ultrahaptics::Units::cm
             && device_palm_position.x <= 28.0f * Ultrahaptics::Units::cm) {
                ...
                local_hand_data.palm_normal = device_palm_normal;
            }
            else
            {
                local_hand_data.palm_position = Ultrahaptics::Vector3();
...
struct ModulatedPoint
{
    ...
    // The position and direction of the control point
    Ultrahaptics::Vector3 position;
    Ultrahaptics::Vector3 point_direction;
                         ...
                         void *user_pointer)
{
    ...
    for (auto& sample : interval)
    {
        ...
        my_modulated_point->point_direction = leapOutput.palm_normal.normalize();

        sample.persistentControlPoint(0).setPosition(my_modulated_point->position);
        sample.persistentControlPoint(0).setDirection(my_modulated_point->point_direction);
...
int main(int argc, char *argv[])
{
    ...
    TimePointStreaming::Emitter emitter("USX:USX-00000000", left_tr);
